 ...
 * @retval 像素数据
 */
uint16_t ILI9341_Read_PixelData ( void )	
{	
...
/**
  ...
  * @param usWidth 区域宽度
  * @param usHeight 区域高度
  * @param usColor 填充颜色数组
  * @retval None
  */
void ILI9341_FillArea(uint16_t usX, uint16_t usY, uint16_t usWidth, uint16_t usHeight, uint16_t* usColors)
{
	ILI9341_OpenWindow(usX, usY, usWidth, usHeight);	
	ILI9341_FillColors((uint32_t)(usWidth * usHeight), usColors);
}

/**
 * @brief  在显示器上按颜色数组填充像素点
 * @param  ulAmout_Point 要填充颜色的像素点的总数目
 * @param  usColors 颜色数组
 * @retval None
 */
void ILI9341_FillColors(uint32_t ulAmout_Point, uint16_t* usColors)
{
	/* memory write */
	ILI9341_Write_Cmd(CMD_SetPixel);
		
	for (uint32_t i = 0; i < ulAmout_Point; i++)
	{
		ILI9341_Write_Data(*usColors);
		++usColors;
	}
}
