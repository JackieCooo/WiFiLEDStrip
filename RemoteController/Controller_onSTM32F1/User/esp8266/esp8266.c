...
/**
  * @brief ESP8266模块发送数据
  * @param enumEnUnvarnishTx 是否使能透传模式
  * @param pBuf 发送数据数组
  * @param ulStrLength 数据长度
  * @param ucId 发送ID
  * @param reply1 正常响应1
  * @param reply2 正常响应2
  * @param waittime 等待时间
  * @return 是否发送成功
  */
bool ESP8266_SendData( FunctionalState enumEnUnvarnishTx, uint8_t* pBuf, uint16_t ulStrLength, ENUM_ID_NO_TypeDef ucId, char* reply1, char* reply2, uint32_t waittime )
{
	char cStr [20];
	bool bRet = false;
	uint8_t* p = pBuf;
	...
	if ( enumEnUnvarnishTx )
	{
		for(uint16_t i = 0; i < ulStrLength; ++i)
		{
			USART_SendByte(macESP8266_USARTx, *p);
			++p;
		}
		bRet = true;
	}
