...
Arduino_ST7789::Arduino_ST7789(int8_t dc, int8_t rst, int8_t sid, int8_t sclk, int8_t cs, int8_t blc)
  : Adafruit_GFX(ST7789_TFTWIDTH, ST7789_TFTHEIGHT)
{
  _cs   = cs;
  _dc   = dc;
...
void Arduino_ST7789::setBackLight(boolean b) {
  if(b) {
    digitalWrite(_blc, HIGH);
  }
  else {
    digitalWrite(_blc, LOW);
  }
}

void Arduino_ST7789::clearScreen(void) {
  fillScreen(BLACK);
}
